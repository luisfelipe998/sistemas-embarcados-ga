...
void rx_task(void *arg)
{
    ...
    while (1)
    {
        ...
        if (rxBytes > 0)
        {
            if (rxBytes == 1 && data[0] == '1')
            {
                // A sequência em data é igual a "1"
                nRetorno = 1; // ou outra ação que você deseja executar
            }

            if (rxBytes == 1 && data[0] == '2')
            {
                // A sequência em data é igual a "1"
                nRetorno = 2; // ou outra ação que você deseja executar
            }

            if (rxBytes == 1 && data[0] == '3')
            {
                // A sequência em data é igual a "1"
                nRetorno = 3; // ou outra ação que você deseja executar
            }
        }
    }
    free(data);
